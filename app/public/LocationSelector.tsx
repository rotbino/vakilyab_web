...
export default function LocationSelector({
    ...
    // Set default to Tehran if no province is selected
    useEffect(() => {
        if (!selectedProvince && !selectedCity) {
            const tehranProvince = provinces.find(p => p.name === "تهران");
            if (tehranProvince) {
                setTempSelectedProvince(tehranProvince.id);
                setTempSelectedCity("تهران");
    ...
    return (
        <>
            <Button
                variant="ghost"
                size="sm"
                className={`h-10 flex items-center gap-1 text-[#ca2a30] ${className} `}
